 ...
 */
@Controller
public class CheckNameController extends ParameterizableViewController
{
	...
	protected ModelAndView handleRequestInternal(HttpServletRequest request, HttpServletResponse response) throws Exception
	{
		...
		UserNameStatus status = userService.checkUserName(name);

		if (status == UserNameStatus.REGISTERED_BUT_MISSPELLED)
			status = UserNameStatus.REGISTERED;

		return new ModelAndView(getViewName(), "status", status.name());
	}
}
